...
export default function ResumenProducto({producto}) {
    ...
    const { HandleEditarCantidad, HandleEdliminarProductoPedido } = useQuiosco()

  return (
    <div className="shadow space-y-1 p-1 bg-white">
        {/* datos del pedido */}
            <div className="space-y-2">
                <p className="text-lg font-bold">{nombre}</p>
                <p className="text-xs font-bold ">Cantidad: {cantidad}</p>
                <p className="text-lxsg font-bold text-amber-500">
      ...
      <div className="flex justify-between gap-2 pb-4">
        ...
        {/* boton eliminar */}
            <button
                type="button"
                onClick={() => HandleEdliminarProductoPedido(id)}
